...
void CostmapLayer::updateWithRemove(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  unsigned char* master = master_grid.getCharMap();
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      if (costmap_[it] == TO_BE_REMOVED)
        master[it] = FREE_SPACE;
      it++;
    }
  }
}
