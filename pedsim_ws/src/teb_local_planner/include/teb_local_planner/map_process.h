...
class mapProcess{
  public:
    /**  main functions  **/
    mapProcess(){};
    // 0.1 form goal extensions with normal navigation
    std::vector<std::vector<Point2D>> normalNvigation_findGoalLineGroups(costmap_2d::Costmap2D* costmap, std::pair<double, double> global_goal, const std::vector<geometry_msgs::PoseStamped>& local_plan);
    // 0.2 form goal extensions with follow mode
    std::vector<std::vector<Point2D>> followNvigation_findGoalLineGroups(costmap_2d::Costmap2D* costmap, std::pair<double, double> global_goal, double safe_distance, double clear_distance, const std::vector<geometry_msgs::PoseStamped>& local_plan);
    // 1. initialize maps derived from costmap
    void initialize(const teb_local_planner::PoseSE2& start, const teb_local_planner::PoseSE2& goal, const std::vector<geometry_msgs::PoseStamped>& local_plan, const std::vector<std::vector<double>>& dynamic_obstacle, std::pair<double,double> velocity_in_map);
    ...
    std::vector<std::vector<Point2D>> getGoalLineList(){return goal_line_lists_;};
    std::vector<Point2D> getGoalCircle(){return goal_circle_;};
  ...
  public:
    ...
    std::vector<Point2D> goal_circle_;
    int safe_distance_cells_;
    int clear_distance_cells_;
    std::string goal_extension_;
    std::string motion_type_;

    double recommended_velocity_;
